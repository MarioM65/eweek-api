...
import { CreateItemSchema, UpdateItemSchema, CreateItemInput, UpdateItemInput } from '../models/Item';
...
export class ItemController {
  ...
  static async create(request: FastifyRequest<{ Body: CreateItemInput }>, reply: FastifyReply) {
    try {
    ...
    /*  if (validatedData.profileId === 3 || validatedData.profileId === 2) {
        const carteira = await CarteiraModel.create({ ItemId: Item.id, credits: 0 });
        ...
        if (validatedData.profileId === 2) {
          ...
          // Relacionar o usuário ao cliente na tabela Item_cliente
          await ItemClienteModel.create({ ItemId: Item.id, clientId: cliente.id });
        }
        ...
        if (validatedData.profileId === 2) {
          ...
          // Relacionar o usuário ao trabalhador na tabela Item_trabalhador
          await ItemTrabalhadorModel.create({ ItemId: Item.id, workerId: trabalhador.id });
        }
      }*/
  ...
  /*static async uploadAvatar(request: FastifyRequest<{ Body: { ItemId: number } }>, reply: FastifyReply) {
    try {
      const data = await request.file(); // Fastify-multipart deve estar configurado
      const ItemId = Number(request.body.ItemId.toString());
      ...
      cloudinary.uploader.upload_stream(
        ...
        async (error, cloudinaryResponse) => {
          ...
          await ItemModel.update(ItemId, { avatarUrl: cloudinaryResponse.secure_url });
  ...
  static async show(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
    try {
      const Item = await ItemModel.findById(parseInt(request.params.id));
      if (!Item) {
  ...
  static async update(request: FastifyRequest<{ Params: { id: string }, Body: UpdateItemInput }>, reply: FastifyReply) {
    try {
      const validatedData = UpdateItemSchema.parse(request.body);
      const Item = await ItemModel.update(parseInt(request.params.id), validatedData);
      return reply.code(200).send({ success: true, data: Item });
    } catch (error) {
      if (error instanceof ZodError) {
        return reply.code(400).send({ success: false, message: 'Validation error', errors: error.errors });
  ...
  static async restore(request: FastifyRequest<{ Params: { id: string }}>, reply: FastifyReply) {
    try {
      const Item = await ItemModel.restore(parseInt(request.params.id));
      return reply.code(200).send({ success: true , data: Item});
    } catch (error) {
      if (error instanceof ZodError) {
        return reply.code(400).send({ success: false, message: 'Validation error', errors: error.errors });
  ...
  static async trash(request: FastifyRequest, reply: FastifyReply) {
    try {
      const Items = await ItemModel.trash();
      return reply.code(200).send({ success: true, data: Items });
    } catch (error) {
      const err = error as Error;
      return reply.code(500).send({ success: false, message: 'Internal server error', error: err.message || 'Unknown error' });
  ...
  static async delete(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
    try {
      await ItemModel.delete(parseInt(request.params.id));
      return reply.code(200).send({ success: true, message: 'Item deleted successfully' });
    } catch (error) {
      const err = error as Error;
      return reply.code(500).send({ success: false, message: 'Internal server error', error: err.message || 'Unknown error' });
    }
  ...
  static async purge(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
    try {
      await ItemModel.purge(parseInt(request.params.id));
      return reply.code(200).send({ success: true, message: 'Item purged successfully' });
    } catch (error) {
      const err = error as Error;
      return reply.code(500).send({ success: false, message: 'Internal server error', error: err.message || 'Unknown error' });
    }
