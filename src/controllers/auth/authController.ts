...
static async loginEntidade(request: FastifyRequest<{ Body: z.infer<typeof loginSchema> }>, reply: FastifyReply) {
  try {
    const { credential, password } = loginSchema.parse(request.body);
    ...
    // Salvar token no banco de dados
    await prisma.entidadeToken.create({
      data: {
        token,
    ...
    return reply.code(200).send({
      ...
      data: {
        ...
        entidade: {
          id: entidade.id,
          vc_email: entidade.vc_email,
          vc_telefone: entidade.vc_telefone,
        }
      }
    });
  } catch (error) {
  ...
  static async logoutEntidade(request: FastifyRequest, reply: FastifyReply) {
    try {
      const authHeader = request.headers.authorization
      if (!authHeader) {
        return reply.code(401).send({
      ...
      // Buscar o token no banco de dados para garantir que ele pertence a um usuário válido
      const entidadeToken = await prisma.entidadeToken.findUnique({
        where: { token }
      })
      ...
      // Revogar o token no banco de dados
      await prisma.entidadeToken.update({
        where: { token },
        data: { revoked: true }
      })
